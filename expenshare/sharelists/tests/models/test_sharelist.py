...
def test_are_in_sharelist(user_factory, sharelist_factory):
    ...
    assert Sharelist.objects.are_in_sharelist(
        sharelist.pk, [user1.pk, user2.pk, user3.pk]
    )
    assert not Sharelist.objects.are_in_sharelist(
        sharelist.pk, [user1.pk, user2.pk, user4.pk]
    )
